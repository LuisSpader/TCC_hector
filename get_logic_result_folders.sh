...
# Output file paths
csv_file="$MY_SCRIPT_PATH/results.csv"
txt_file="$MY_SCRIPT_PATH/results.txt"
...
get_power_results() {
	...
	# Search string to identify the section of interest
	# local words=("Instance" "Cell" "Power")
	# local words=("Category" "Leakage" "Internal" "Switching" "Total" "Row")
	local words=("Subtotal")
	local pattern=$(create_pattern "${words[@]}")
	# Extract the relevant section
	#relevant_section=$(grep -A2 "$search_string" <<< "$text")
	# local relevant_section=$(grep -A2 -E $pattern <<< "$text" )
	local relevant_section=$(grep Subtotal <<< "$text" )
	...
	local leakage=$(awk '{print $2}' <<< "$relevant_section")
	local internal=$(awk '{print $3}' <<< "$relevant_section")
	local switching=$(awk '{print $4}' <<< "$relevant_section")
	local total_power=$(awk '{print $5}' <<< "$relevant_section")

	# Print the values
	#echo "Number of Cells: $number_of_cells"
	#echo "Leakage Power: $leakage_power nW"
	#echo "Dynamic Power: $switching nW"
	#echo "Total Power: $total_power nW"

	#echo -e "$folder_name\t\t$leakage_power\t\t$switching\t\t$total_power" >> "$txt_file"
	echo "$leakage:$internal:$switching:$total_power"
}
...
do
	...
	IFS=':' read -r leakage_power internal switching total_power <<< "$result"
	# $leakage:$internal:$switching:$total_power
	#echo -e "$folder_name\t\t$leakage_power\t\t$switching\t\t$total_power" >> "$txt_file"


	# Append the results to the CSV file
	echo "$folder_name, $number_of_cells, $cell_area, $net_area, $total_area, $highest_timing, $start_point, $end_point, $leakage_power, $internal, $switching, $total_power" >> "$csv_file"
...
# echo -e "Folder\t\t\t\t\t\tNumber of Cells\t\tCell Area\t\tNet Area\t\tTotal Area"
...
do
	...
	# Read the text from a file
	text=$(cat ./synthesis/synthesis_log.txt)
	# echo "text= ./synthesis/synthesis_log.txt"
...
# echo -e "\nFolder\t\t\t\t\t\tHighest Timing Value\tStart-point\t\tEnd-point"
# Loop over the sorted folders
for dir in $folders
do
	# Print the current directory
...
echo "# ------------------------------- GET POWER ------------------------------- #"
# Create header in txt file as a table
echo -e "\nFolder\t\t\t\t\t\t\tLeakage Power\t\tInternal\t\tDynamic Power\t\tTotal Power" >> "$txt_file"
# echo -e "\nFolder\t\t\t\t\t\tLeakage Power\t\tInternal\t\tDynamic Power\t\tTotal Power"
# Loop over the sorted folders
for dir in $folders
do
	# Print the current directory
	...
	result=$(get_power_results "$text" "$folder_name" )
	IFS=':' read -r leakage_power internal switching total_power <<< "$result"
	# echo -e "$folder_name\t\t$leakage_power\t\t$internal\t\t$switching\t\t$total_power" 
	echo -e "$folder_name\t\t\t$leakage_power\t\t\t$internal\t\t$switching\t\t\t$total_power" >> "$txt_file"
	# $leakage:$internal:$switching:$total_power"
